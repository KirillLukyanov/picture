...
const filter = () => {
  ...
  menu.addEventListener('click', e => {
    ...
    if (target && target.tagName == 'LI') {
      ...
      if (target.className == 'grandmother' || target.className == 'granddad') {
        no.style.display = 'block';
        no.classList.add('animated', 'fadeIn');
      } else {
        wrapper.querySelectorAll(`.${target.className}`).forEach(item => {
...
const mask = selector => {
  ...
  function createMask(event) {
    let matrix = '+7 (___) ___ __ __',
        ...
        // значение, полученное на основе матрицы
    val = this.value.replace(/\D/g, ''); // значение, полученное на основе того, что ввел пользователь

    if (def.length >= val.length) {
      // не даем пользователю удалить +7
      val = def;
    }

    this.value = matrix.replace(/./g, function (a) {
      // Проходимся по всем (/./) символам в матрице и заменяем "_" на значения, которые вводит пользователь
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('http://localhost:3000/styles').then(res => createCards(res)).catch(err => console.log(err));
    this.remove();
  });
